...
<body>
	...
	<div class="pagination">
		<ul>
			<c:if test="${pageMaker.prev }">
				<li><a
					href="regList${pageMaker.makeSearch(pageMaker.startPage - 1)}">이전</a></li>
			</c:if>

			<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage }"
				var="idx">
				<li><a href="regList${pageMaker.makeSearch(idx)}">${idx}</a></li>
			</c:forEach>

			<c:if test="${pageMaker.next && pageMaker.endPage > 0 }">
				<li><a
					href="regList${pageMaker.makeSearch(pageMaker.endPage + 1)}">다음</a></li>
			</c:if>
	...
	<script>
		$(function() {
			$('#searchBtn').click(
					function() {
						self.location = "regList" + '${pageMaker.makeQuery(1)}'
								+ "&searchType="
