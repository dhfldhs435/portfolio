			...
			<c:otherwise>
				<c:forEach items="${regAppList }" var="creationlist">
					<tr>
						...
						<td align="left"><a
							href="regInfo?e_num=${creationlist.e_num}">${creationlist.e_title }</a></td>
						<td align="center"><img
	...
	<div class="pagination">
		<ul>
			<c:if test="${pageMaker.prev }">
				<li><a
					href="regAppList${pageMaker.makeSearch(pageMaker.startPage - 1)}">이전</a></li>
			</c:if>

			<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage }"
				var="idx">
				<li><a href="regAppList${pageMaker.makeSearch(idx)}">${idx}</a></li>
			</c:forEach>

			<c:if test="${pageMaker.next && pageMaker.endPage > 0 }">
				<li><a
					href="regAppList${pageMaker.makeSearch(pageMaker.endPage + 1)}">다음</a></li>
			</c:if>
