...
<body>
	<h1 style="position: static; right: 50%;">일반회원 공지사항</h1>
	...
	<div class="search">
		<select name="searchType">
			...
			<option value="mn"
				<c:out value="${scri.searchType eq 'mn' ? 'selected' : ''}"/>>공지번호</option>
			<option value="mt"
				<c:out value="${scri.searchType eq 'mt' ? 'selected' : ''}"/>>제목</option>
			<option value="mw"
				<c:out value="${scri.searchType eq 'mw' ? 'selected' : ''}"/>>작성자</option>
			<option value="mtmw"
				<c:out value="${scri.searchType eq 'mtmw' ? 'selected' : ''}"/>>제목+작성자</option>
			...
			<c:otherwise>
				<c:forEach items="${noticeListM }" var="notice">
					<tr>
						...
						<td align="left"><a
							href="memberRead?mn_num=${notice.mn_num }"><c:out
	...
	<div class="pagination">
		<ul>
			<c:if test="${pageMaker.prev }">
				<li><a
					href="memberList${pageMaker.makeSearch(pageMaker.startPage - 1)}">이전</a></li>
			</c:if>
			...
			<c:if test="${pageMaker.next && pageMaker.endPage > 0 }">
				<li><a
					href="memberList${pageMaker.makeSearch(pageMaker.endPage + 1)}">다음</a></li>
			</c:if>
