...
<form:form modelAttribute="notice">
	...
	<table border="1" align="center" class="member">
		...
		<tr>
			...
			<td colspan="3"><form:input path="sn_title" size="150"
					readonly="true" /></td>
		</tr>
		<tr>
			<td><spring:message code="notice.content" /></td>
			<td align="center" colspan="3"><form:textarea path="sn_content"
					cols="150" rows="25" readonly="true" /></td>
		</tr>
		<tr>
			<td><spring:message code="notice.attachmentfile" /></td>
			<c:choose>
				<c:when test="${empty notice.sn_attachment_name }">
					<td colspan="3"><spring:message code="common.attachmentEmpty" /></td>
				</c:when>
				<c:otherwise>
					<td colspan="3"><a
						href="/admin/notice/sellerdownload\?sn_num=${notice.sn_num }"><form:input
								path="sn_attachment_name" size="80" readonly="true" /></a></td>
				</c:otherwise>
			</c:choose>
		</tr>
	</table>
...
<script>
	$(document).ready(function() {
		...
		$("#btnModify").on("click", function() {
			...
			self.location = "/admin/notice/sellerModify?sn_num=" + sn_numVal;
		});
		$("#btnRemove").on("click", function() {
			formObj.attr("action", "/admin/notice/sellerRemove");
			formObj.submit();
		});
